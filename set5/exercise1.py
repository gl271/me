...
def wordy_pyramid():
    ...
    pyramid_list = list_of_words_with_lengths(listLength)
...
def get_a_word_of_length_n(length):
    baseURL = (
        ...
        "give_me_a_word?wordlength={lenUrl}"
    )

    url = baseURL.format(lenUrl=length)
    r = requests.get(url)
    if r.status_code == 200:
        message = r.text
    else:
        message = "failed a request" + " " + r.status_code + " " + i
    return message
...
def list_of_words_with_lengths(list_of_lengths):
    ...
    for length in list_of_lengths:
        wordList.append(get_a_word_of_length_n(length))
    return wordList
